...
// Hover state from editor - grey outline with dashes
g.red-ui-flow-node-hovered:not(.red-ui-flow-node-selected) {
...
// Hover state from outline - grey outline with dashes (only when not selected)
g.red-ui-flow-node-outline-hovered:not(.red-ui-flow-node-selected) {
    .red-ui-flow-node,.red-ui-flow-subflow-port {
        stroke-width: 3;
        stroke: #999 !important;
        stroke-dasharray: 8, 4;
...
// Selected state always takes precedence
