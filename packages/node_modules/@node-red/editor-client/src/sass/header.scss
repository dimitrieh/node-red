...
#red-ui-header {
    ...
    .red-ui-deploy-button {
        ...
        // Modern Deploy button styling
        @include mixins.modern-border-radius(3px);
        @include mixins.smooth-transition(all, 0.15s);
        font-weight: 500; // Slightly bolder for better visibility
        letter-spacing: 0.025em; // Subtle letter spacing for professional feel

        &:hover {
            background: var(--red-ui-deploy-button-background-hover);
            transform: translateY(-1px); // Consistent with other buttons
            @include mixins.modern-shadow-subtle;
        }
        &:focus {
            outline: none;
