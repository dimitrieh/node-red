...
#red-ui-header {
    ...
    // 2025 Modern Header Buttons - Refined Interaction Design
    .button {
        height: 100%;
        display: inline-flex;
        align-items: center;
        ...
        border: 2px solid transparent; // Simplified borders
    ...
    // 2025 Modern Deploy Button - Enhanced Call-to-Action Design
    .red-ui-deploy-button {
        ...
        font-weight: 600; // Stronger visual authority
        @include mixins.transition-smooth;
        transition-property: all;
        @include mixins.shadow-subtle; // Subtle depth
        position: relative;
        overflow: hidden;
        ...
        &:hover {
            ...
            @include mixins.shadow-soft; // Enhanced depth on hover
        }
        
        &:focus {
            outline: none;
            @include mixins.focus-ring-modern;
        }
        
        &:active {
            background: var(--red-ui-deploy-button-background-active);
            color: var(--red-ui-deploy-button-color-active);
            transform: translateY(0) scale(0.98); // Press feedback
        }
        
        // Modern shimmer effect on successful deploy (optional enhancement)
        &.deploy-success::after {
            content: '';
            position: absolute;
            top: 0;
            left: -100%;
