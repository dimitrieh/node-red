...
#red-ui-workspace {
    ...
    // Modern workspace refinements - FUNCTIONALITY PRESERVED
    @include mixins.smooth-transition(left, 0.1s);
    background: var(--red-ui-view-background); // Ensure consistent background
}
...
.red-ui-workspace-chart-grid line {
    ...
    // Subtle modern industrial grid refinement - PRESERVES functionality
    opacity: 0.8; // Slightly more subtle, but still visible for technical precision
    stroke-dasharray: none; // Keep solid lines for professional industrial look
}
