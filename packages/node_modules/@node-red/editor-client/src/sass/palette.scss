...
#red-ui-palette{
    ...
    // Mobile: Hide palette by default, show as overlay when needed
    @include mixins.mobile-only {
        @include mixins.mobile-panel-overlay;
        width: 280px; // Wider for easier touch interaction
        z-index: 1001;
        
        &:before {
            display: none; // Hide grip on mobile
        }
    }

    // Tablet: Slightly narrower palette
...
.red-ui-palette-search {
    ...
    // Mobile: Larger touch-friendly search input
    @include mixins.mobile-only {
        height: 44px;
        padding: 6px;
...
.red-ui-palette-header {
    ...
    // Mobile: Touch-friendly category headers
    @include mixins.mobile-only {
        @include mixins.touch-friendly-button;
        padding: 12px;
        padding-left: 40px;
        min-height: 44px;
        font-size: 16px;
...
.red-ui-palette-node {
    ...
    // Mobile: Larger, more touch-friendly nodes
    @include mixins.mobile-only {
        height: 36px;
        width: 200px; // Wider for better touch interaction
        margin: 12px auto;
        cursor: pointer; // Better for touch devices
        
        // Larger touch targets
        &:not(.red-ui-palette-node-config):not(.red-ui-palette-node-small) {
            margin-top: 16px;
            margin-bottom: 16px;
        }
    }

    // Tablet: Slightly larger nodes
    @include mixins.tablet-only {
        height: 28px;
        width: 140px;
        margin: 12px auto;
