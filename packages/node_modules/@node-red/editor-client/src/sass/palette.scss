...
#red-ui-palette{
    ...
    // Mobile optimization: Hide palette on very small screens by default
    @media screen and (max-width: 480px) {
        width: 8px; // Collapsed by default on mobile
    }
    
    // Tablet optimization: Narrower palette
    @media screen and (min-width: 481px) and (max-width: 768px) {
        width: 140px; // Narrower on tablets to save space
    }
...
.red-ui-palette-node {
    ...
    @media screen and (max-width: 768px) {
        height: 40px; // Larger height for finger tapping
        margin: 12px auto; // More spacing between nodes
        width: 100%; // Use full width on mobile
        max-width: 140px; // But don't exceed reasonable size
        min-height: 44px; // Apple's recommended minimum touch target
    }
    
    @media screen and (max-width: 480px) {
        height: 44px; // Even larger on small phones
        margin: 15px auto; // More generous spacing
