...
RED.nodes = (function() {
    ...
    function addNode(n, opt) {
        ...
        // Sync to Automerge if enabled and this is not from remote
        if (opt && !opt.fromRemote && RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.addNode(nodeToExportable(newNode));
        }
    ...
    /**
     * Convert a node to exportable format (for Automerge sync)
     */
    function nodeToExportable(n) {
        if (!n) return null;
        var node = {};
        node.id = n.id;
        node.type = n.type;
        if (n.type !== "tab" && n.type !== "subflow") {
            ...
            node.y = n.y;
            if (n.wires) {
                node.wires = n.wires.map(function(w) {
                    return w.map(function(link) {
        ...
        for (var d in n._def.defaults) {
            if (n._def.defaults.hasOwnProperty(d) && d !== '_') {
                node[d] = n[d];
            }
        }
        if (n.type === "tab") {
            node.label = n.label;
            node.disabled = n.disabled;
        ...
        if (n.type === "group") {
            node.name = n.name;
            node.style = n.style;
            node.nodes = n.nodes ? n.nodes.map(function(gn) { return gn.id || gn; }) : [];
            node.x = n.x;
            node.y = n.y;
            node.w = n.w;
            node.h = n.h;
    ...
    function addLink(l) {
        ...
        // Sync wire to Automerge if enabled
        if (RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.addWire(l.source.id, l.sourcePort, l.target.id);
        }
    }
    ...
    function removeNode(id) {
        ...
        // Sync to Automerge if enabled
        if (node && RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.removeNode(id, node.type);
        }
    ...
    function removeLink(l) {
        ...
        // Sync wire removal to Automerge if enabled
        if (RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.removeWire(l.source.id, l.sourcePort, l.target.id);
        }
    }
    ...
    function addWorkspace(ws,targetIndex) {
        ...
        // Sync workspace to Automerge if enabled
        if (RED.automerge && RED.automerge.isEnabled()) {
    ...
    function addGroup(group) {
        ...
        // Sync group to Automerge if enabled
        if (RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.addNode({
                id: group.id,
                type: 'group',
                ...
                name: group.name,
                style: group.style,
                nodes: group.nodes ? group.nodes.map(function(n) { return n.id || n; }) : [],
                x: group.x,
                y: group.y,
                w: group.w,
                h: group.h
    ...
    function removeGroup(group) {
        ...
        // Sync group removal to Automerge if enabled
        if (RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.removeNode(group.id, 'group');
        }
    }
    ...
    return {
        ...
        nodeToExportable: nodeToExportable,
        id: getID,
        dirty: function(d) {
            if (d == null) {
                return dirty;
