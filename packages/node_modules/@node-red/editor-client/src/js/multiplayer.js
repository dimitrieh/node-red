RED.multiplayer = (function () {
    ...
    function updateUserLocation (sessionId, location) {
        ...
        if (location.workspace) {
            ...
            if (location.cursor && location.workspace === RED.workspaces.active()) {
                const inverseScale = 1 / RED.view.scale()
                if (!sessions[sessionId].cursor) {
                    ...
                    cursorIcon.appendChild(createCursorLabel(user))
                    $(cursorIcon).css({
                        transform: `translate(${location.cursor.x}px, ${location.cursor.y}px) scale(${inverseScale})`,
                        transition: 'transform 0.1s ease-out'
                    })
                    $(".red-ui-workspace-chart-event-layer").append(cursorIcon)
                    sessions[sessionId].cursor = cursorIcon
                    resetCursorTimers(sessionId)
                } else {
    ...
    function createCursorLabel(user) {
        ...
        labelGroup.setAttribute("class", "red-ui-multiplayer-cursor-label")
        labelGroup.setAttribute("transform", "translate(12, 16)")
        const bg = document.createElementNS("http://www.w3.org/2000/svg","rect");
        bg.setAttribute("class", "red-ui-multiplayer-cursor-label-bg")
        if (user && user.profileColor !== undefined) {
            bg.classList.add("red-ui-user-profile-color-" + user.profileColor)
        }
        ...
        text.setAttribute("class", "red-ui-multiplayer-cursor-label-text")
        text.setAttribute("x", "4")
        text.setAttribute("y", "13")
        text.textContent = user.username || user.email || 'anonymous'
    ...
    const cursorTimers = {}
    function resetCursorTimers(sessionId) {
        if (cursorTimers[sessionId]) {
            clearTimeout(cursorTimers[sessionId].fadeTimeout)
            clearTimeout(cursorTimers[sessionId].hideTimeout)
        }
        if (sessions[sessionId]?.cursor) {
            $(sessions[sessionId].cursor).removeClass('red-ui-multiplayer-cursor-faded red-ui-multiplayer-cursor-hidden')
        }
        cursorTimers[sessionId] = {
            fadeTimeout: setTimeout(() => {
                if (sessions[sessionId]?.cursor) {
                    $(sessions[sessionId].cursor).addClass('red-ui-multiplayer-cursor-faded')
                }
                cursorTimers[sessionId].hideTimeout = setTimeout(() => {
                    if (sessions[sessionId]?.cursor) {
                        $(sessions[sessionId].cursor).addClass('red-ui-multiplayer-cursor-hidden')
                    }
                }, 25000)
            }, 5000)
        }
    }
    function clearCursorTimers(sessionId) {
        if (cursorTimers[sessionId]) {
            clearTimeout(cursorTimers[sessionId].fadeTimeout)
            clearTimeout(cursorTimers[sessionId].hideTimeout)
            delete cursorTimers[sessionId]
        }
    }
    ...
    return {
        ...
        init: function () {
            ...
            RED.events.on('workspace:change', (event) => {
                // Remove all remote cursors (they belong to old workspace view)
                for (const sessionId of Object.keys(sessions)) {
                    if (sessionId !== activeSessionId && sessions[sessionId].cursor) {
                        removeUserCursor(sessionId)
                    }
                }
                // Re-create cursors for users on the new active workspace
                for (const sessionId of Object.keys(sessions)) {
                    if (sessionId !== activeSessionId) {
                        const loc = sessions[sessionId].location
                        if (loc && loc.workspace === event.workspace && loc.cursor) {
                            updateUserLocation(sessionId)
                        }
                    }
                }
            ...
            RED.events.on('view:zoom', (event) => {
                const inverseScale = 1 / event.scale
                for (const sessionId of Object.keys(sessions)) {
                    if (sessions[sessionId].cursor) {
                        const loc = sessions[sessionId].location
                        if (loc && loc.cursor) {
                            $(sessions[sessionId].cursor).css({
                                transform: `translate(${loc.cursor.x}px, ${loc.cursor.y}px) scale(${inverseScale})`
                            })
                        }
                    }
                }
