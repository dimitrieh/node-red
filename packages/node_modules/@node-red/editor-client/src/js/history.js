...
RED.history = (function() {
    ...
    return {
        ...
        push: function(ev) {
            ...
            // Create Automerge checkpoint if collaborative editing is enabled
            if (RED.automerge && RED.automerge.isEnabled()) {
                RED.automerge.createCheckpoint();
            }
        },
        pop: function() {
            ...
            // Sync current state to Automerge after undo
            if (RED.automerge && RED.automerge.isEnabled()) {
                // The undoEvent has already modified the editor state
                // Now sync the current state to Automerge
                var currentFlows = RED.nodes.createCompleteNodeSet();
                RED.automerge.setFlows(currentFlows);
            }
        },
        peek: function() {
        ...
        redo: function() {
            ...
            // Sync current state to Automerge after redo
            if (RED.automerge && RED.automerge.isEnabled()) {
                var currentFlows = RED.nodes.createCompleteNodeSet();
                RED.automerge.setFlows(currentFlows);
            }
