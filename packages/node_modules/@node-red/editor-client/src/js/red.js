...
var RED = (function() {
    ...
    function loadEditor() {
        ...
        // Initialize floating windows system and convert palette
        if (RED.floatingWindows) {
            // Create floating window for palette
            var paletteWindow = RED.floatingWindows.createWindow({
                title: 'Node Palette',
                x: 20,
                y: 60,
                width: 250,
            ...
            // Move palette content into floating window
            var paletteContent = $('#red-ui-palette').detach();
            RED.floatingWindows.setWindowContent(paletteWindow.id, paletteContent);
            RED.floatingWindows.showWindow(paletteWindow.id);
            
            // Store reference for palette toggle functionality
            RED.palette._floatingWindow = paletteWindow;
        }
        ...
        if (RED.floatingWindows) {
            // Create floating window for sidebar
            var sidebarWindow = RED.floatingWindows.createWindow({
                title: 'Information',
                x: 300,
                y: 60,
                width: 350,
            ...
            // Move sidebar content into floating window
            var sidebarContent = $('#red-ui-sidebar').detach();
            RED.floatingWindows.setWindowContent(sidebarWindow.id, sidebarContent);
            RED.floatingWindows.showWindow(sidebarWindow.id);
            
            // Store reference for sidebar toggle functionality
            RED.sidebar._floatingWindow = sidebarWindow;
        }
    ...
    function buildEditor(options) {
        ...
        // Create palette and sidebar as detached elements to be wrapped in floating windows later
        $('<div id="red-ui-palette"></div>').appendTo('body').hide();
        $('<div id="red-ui-sidebar"></div>').appendTo('body').hide();
