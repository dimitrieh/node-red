        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var stepConfig = function() {
                    ...
                    if (configs.length === 0) {
                        $("#red-ui-editor").i18n();
                        $("#red-ui-left-sidebar-palette-tab .red-ui-palette-spinner, #red-ui-palette > .red-ui-palette-spinner").hide();
                        $(".red-ui-palette-scroll").removeClass("hide");
                        $("#red-ui-left-sidebar-palette-tab #red-ui-palette-search, #red-ui-palette-search").removeClass("hide");
        ...
        $.getJSON(options.apiRootUrl+"theme", function(theme) {
            if (theme.header) {
                ...
                if (theme.header.title) {
                    // Enhanced header with dashboard link and GitHub linking
                    var headerContainer = $('<span>').appendTo(logo);
                    
                    // If URL is provided, wrap the title in a link
                    if (theme.header.url) {
                        // Create link for the title
                        var titleLink = $('<a>', {
                            href: theme.header.url,
                            target: '_blank',
                            text: theme.header.title,
                            style: 'color: inherit; text-decoration: underline;'
                        });
                        headerContainer.append(titleLink);
                    } else {
                        // No URL provided, just show the title
