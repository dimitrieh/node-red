    ...
    function show(options) {
        ...
        if (options.options) {
            ...
            if (hasSelection && canEdit) {
                ...
                // Add annotation menu items for single node selection
                if (!hasMultipleSelection) {
                    const selectedNode = selection.nodes[0];
                    const hasAnnotation = RED.view.annotations && RED.view.annotations.hasUserAnnotation && RED.view.annotations.hasUserAnnotation(selectedNode);
                        ...
                        null,
                        {
                            onselect: function() {
                                if (RED.view.annotations && RED.view.annotations.setUserAnnotation) {
                                    const currentText = RED.view.annotations.hasUserAnnotation(selectedNode) 
                                        ? RED.view.annotations.getUserAnnotation(selectedNode).text 
                                        : '';
                                    const newText = prompt(hasAnnotation ? 'Edit annotation:' : 'Add annotation:', currentText);
                                    if (newText !== null) {
                                        RED.view.annotations.setUserAnnotation(selectedNode, newText);
                                    }
                                }
                            },
                            label: hasAnnotation ? 'Edit Annotation' : 'Add Annotation'
                        }
                    );
                    ...
                    if (hasAnnotation) {
                        nodeOptions.push({
                            onselect: function() {
                                if (RED.view.annotations && RED.view.annotations.setUserAnnotation) {
                                    RED.view.annotations.setUserAnnotation(selectedNode, '');
                                }
                            },
