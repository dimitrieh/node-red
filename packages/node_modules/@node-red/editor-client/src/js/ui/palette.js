...
RED.palette = (function() {
    ...
    function createCategoryContainer(originalCategory,category, labelId) {
        ...
        var catDiv = $('<div id="red-ui-palette-container-'+category+'" class="red-ui-palette-category hide">'+
            '<div id="red-ui-palette-header-'+category+'" class="red-ui-palette-header"><span>'+label+'</span></div>'+
        ...
        // Category headers are no longer clickable - collapsible functionality removed
    }

    function setLabel(type, el,label, info) {
        var nodeWidth = 82;
        var nodeHeight = 30; // Match editor node height
        var lineHeight = 16; // Adjusted for 14px font
        var portHeight = 10;
    ...
    function addNodeType(nt,def) {
        ...
        if (exclusion.indexOf(nodeCategory)===-1) {
            ...
            if (categoryNode.find(".red-ui-palette-node").length === 1) {
                // Always show categories since they're no longer collapsible
    ...
    function init() {
        ...
        // Add resize handle
        $('<div class="red-ui-palette-resize-handle"></div>').appendTo("#red-ui-palette");
        
        // Move search to the header center section
        $('<div class="red-ui-header-center"><div id="red-ui-palette-search" class="red-ui-palette-search"><input type="text" data-i18n="[placeholder]palette.filter"></input></div></div>').appendTo("#red-ui-header");
        
        // Add toggle button for palette collapse/expand - positioned near resize handle
        toggleButton = $('<div class="red-ui-palette-toggle-button"><i class="fa fa-chevron-up"></i></div>').appendTo("#red-ui-palette");
        RED.popover.tooltip(toggleButton, RED._("keyboard.togglePalette") || "Toggle Palette", "core:toggle-palette");
        ...
        // Add hover behavior for toggle button (similar to sidebar)
        $("#red-ui-palette").on("mouseenter", function() {
            if (!paletteCollapsed) {
                toggleButton.toggle("slide", { direction: "down" }, 200);
            }
        });
        $("#red-ui-palette").on("mouseleave", function() {
            if (!paletteCollapsed) {
                toggleButton.stop(false, true);
                toggleButton.hide();
            }
        });
        ...
        // Old sidebar controls removed - replaced with new toggle button system
        var userCategories = [];
        ...
        // Palette collapse/expand functionality - now handled by auto-collapse in resize logic
        var paletteCollapsed = false;
        var minHeight = 120; // Minimum height before auto-collapse
        var uncollapseButton;
        var toggleButton;

        // Add uncollapse button (initially hidden)
        uncollapseButton = $('<div class="red-ui-palette-uncollapse-button"><i class="fa fa-chevron-down"></i></div>').appendTo("#red-ui-palette");
        RED.popover.tooltip(uncollapseButton, "Expand Palette");
        uncollapseButton.hide();
        
        uncollapseButton.on("click", function(e) {
            e.preventDefault();
            expandPalette();
        });

        // Add draggable resize functionality
        var isResizing = false;
        var startY = 0;
        var startHeight = 0;
        var currentHeight = 200; // Default height
        var animationFrame = null;
        
        // Load saved height from localStorage
        try {
            var savedHeight = RED.settings.getLocal("palette-height");
            if (savedHeight && !isNaN(savedHeight)) {
                currentHeight = parseInt(savedHeight);
                $("#red-ui-palette").css("height", currentHeight + "px");
                $("#red-ui-main-container").css("top", "calc(var(--red-ui-header-height) + " + currentHeight + "px)");
            }
        } catch (e) {
        ...
        function collapsePalette() {
            ...
            $("#red-ui-palette").addClass("collapsed");
            $("#red-ui-main-container").css("top", "calc(var(--red-ui-header-height) + 8px)");
            uncollapseButton.show();
        ...
        function expandPalette() {
            ...
            $("#red-ui-palette").removeClass("collapsed");
            $("#red-ui-palette").css("height", currentHeight + "px");
            $("#red-ui-main-container").css("top", "calc(var(--red-ui-header-height) + " + currentHeight + "px)");
            uncollapseButton.hide();
            // Update toggle button icon
        ...
        function updatePaletteHeight(newHeight, immediate) {
            // Auto-collapse if height becomes too small
            if (newHeight < minHeight && !paletteCollapsed) {
                collapsePalette();
                return;
            }
            
            // Don't update height if collapsed
            if (paletteCollapsed) {
                return;
            }
            
            if (immediate) {
                // Direct update for dragging - no animation frame needed
                $("#red-ui-palette").css("height", newHeight + "px");
                $("#red-ui-main-container").css("top", "calc(var(--red-ui-header-height) + " + newHeight + "px)");
                currentHeight = newHeight;
            } else {
                // Use animation frame for non-dragging updates
                if (animationFrame) {
        ...
        $(".red-ui-palette-resize-handle").on("mousedown", function(e) {
            // Don't allow resizing if palette is collapsed
            if (paletteCollapsed) {
                return;
            }
            ...
            isResizing = true;
            startY = e.clientY;
            startHeight = $("#red-ui-palette").height();
            
            // Add body class to prevent text selection during resize
            $(document.body).addClass("red-ui-palette-resizing");
            
            $(document).on("mousemove.palette-resize", function(e) {
                if (!isResizing || paletteCollapsed) return;
                
                var deltaY = e.clientY - startY;
                var newHeight = Math.max(minHeight, Math.min(400, startHeight + deltaY));
                
                // Use immediate update during drag to reduce delay and visual artifacts
                updatePaletteHeight(newHeight, true);
            });
            
            $(document).on("mouseup.palette-resize", function() {
                isResizing = false;
                $(document).off("mousemove.palette-resize mouseup.palette-resize");
                $(document.body).removeClass("red-ui-palette-resizing");
                ...
                // Save height to localStorage only if not collapsed
                if (!paletteCollapsed) {
                    try {
                        RED.settings.setLocal("palette-height", currentHeight.toString());
                    } catch (e) {
                        // Ignore localStorage errors
                    }
                }
