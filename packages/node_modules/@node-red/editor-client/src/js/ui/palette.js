...
RED.palette = (function() {
    ...
    var isFloatingMode = true; // Enable floating mode by default
    ...
    function init() {
        ...
        if (isFloatingMode) {
            // Initialize floating window infrastructure
            RED.floatingWindow.init();

            // Create floating palette window
            paletteWindow = RED.floatingWindow.create({
                id: 'node-palette-window',
                title: 'Node Palette',
                position: { x: 20, y: 80 },
            ...
            // Create palette content in floating window
            const paletteContent = $('<div id="red-ui-palette-floating" style="height: 100%; display: flex; flex-direction: column;"></div>');
            
            paletteContent.append('<img src="red/images/spin.svg" class="red-ui-palette-spinner hide"/>');
            paletteContent.append('<div id="red-ui-palette-search" class="red-ui-palette-search" style="padding: 8px;"><input type="text" data-i18n="[placeholder]palette.filter" style="width: 100%; padding: 6px; border: 1px solid #ccc; border-radius: 3px;"></input></div>');
            paletteContent.append('<div id="red-ui-palette-container" class="red-ui-palette-scroll" style="flex: 1; overflow-y: auto; padding: 0 8px;"></div>');
            paletteContent.append('<div class="red-ui-component-footer" style="border-top: 1px solid #eee; padding: 8px;"></div>');
            paletteContent.append('<div id="red-ui-palette-shade" class="hide"></div>');

            paletteWindow.setContent(paletteContent);
            paletteContent.find(".red-ui-palette-spinner").show();

        } else {
            // Original fixed palette mode
    ...
    function togglePalette(state) {
        if (isFloatingMode && paletteWindow) {
            if (state === undefined) {
            ...
            // Original fixed palette behavior
            if (!state) {
