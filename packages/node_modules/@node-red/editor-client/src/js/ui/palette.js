...
RED.palette = (function() {
    ...
    var palettePosition = { x: 50, y: 50, width: 220, height: 400 };
    ...
    function init() {
        ...
        RED.actions.add("core:toggle-palette-floating", function() {
            toggleFloatingMode();
        });
        ...
        try {
            palettePosition = JSON.parse(RED.settings.getLocal("palette-floating-position") || JSON.stringify(palettePosition));
        } catch (error) {
            console.error("Unexpected error loading palette position from localStorage: ", error);
        }
    ...
    function saveFloatingPosition() {
        try {
            RED.settings.setLocal("palette-floating-position", JSON.stringify(palettePosition));
        } catch (error) {
            console.error("Unexpected error saving palette position to localStorage: ", error);
        }
    }
    ...
    function createFloatingWindow() {
        ...
        paletteWindow = $('<div id="red-ui-palette-floating" class="red-ui-floating-window"></div>')
            .appendTo('#red-ui-main-container');
            
        // Add window header with controls
        var header = $('<div class="red-ui-floating-window-header"></div>').appendTo(paletteWindow);
        $('<span class="red-ui-floating-window-title">Palette</span>').appendTo(header);
        var controls = $('<div class="red-ui-floating-window-controls"></div>').appendTo(header);
        
        // Dock button
        var dockBtn = $('<button class="red-ui-floating-window-button"><i class="fa fa-window-minimize"></i></button>')
            .appendTo(controls)
            .on('click', function() {
                toggleFloatingMode();
            });
        RED.popover.tooltip(dockBtn, "Dock palette");
        
        // Move the palette content to floating window
        var paletteContent = $('#red-ui-palette').detach();
        paletteContent.removeClass().addClass('red-ui-palette-floating-content');
        paletteWindow.append(paletteContent);
        ...
        paletteWindow.css({
            ...
            height: palettePosition.height + 'px',
            zIndex: 1000
        });
        ...
        paletteWindow.draggable({
            ...
            stop: function(event, ui) {
                palettePosition.x = ui.position.left;
                palettePosition.y = ui.position.top;
                saveFloatingPosition();
            }
        });
        
        paletteWindow.resizable({
            minWidth: 180,
            minHeight: 200,
            handles: 'n,e,s,w,ne,nw,se,sw',
            stop: function(event, ui) {
                palettePosition.width = ui.size.width;
                palettePosition.height = ui.size.height;
                saveFloatingPosition();
            }
        });
    }
    ...
    function destroyFloatingWindow() {
        ...
        // Move palette content back to original position
        var paletteContent = paletteWindow.find('.red-ui-palette-floating-content');
        paletteContent.removeClass().attr('id', 'red-ui-palette');
        $('#red-ui-main-container').append(paletteContent);
    ...
    function toggleFloatingMode() {
        ...
        if (floatingMode) {
            ...
            $('#red-ui-main-container').addClass('red-ui-palette-floating');
            $('#red-ui-main-container').removeClass('red-ui-palette-closed');
        } else {
            // Switch to docked mode  
            destroyFloatingWindow();
            $('#red-ui-main-container').removeClass('red-ui-palette-floating');
        ...
        // Trigger resize to adjust workspace
        setTimeout(function() { $(window).trigger('resize'); }, 200);
    }
    ...
    return {
        ...
        toggleFloating: toggleFloatingMode
    };
})();
