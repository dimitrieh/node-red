...
RED.sidebar.info = (function() {
    ...
    function refresh(node) {
        ...
        if (node === null) {
            ...
            if (RED.view.annotations && RED.view.annotations.hasUserAnnotation && RED.view.annotations.hasUserAnnotation(node)) {
                var annotationContainer = $("<div>").css("padding","6px").appendTo(propertiesPanelContent);
                var annotationHeader = $("<h3>").css({
                    "color": "#666",
                    "font-size": "0.9em",
                    "margin": "0 0 8px 0",
                    "border-bottom": "1px solid #ddd",
                    "padding-bottom": "4px"
                }).text("📝 Annotation").appendTo(annotationContainer);
                
                var annotation = RED.view.annotations.getUserAnnotation(node);
                if (annotation && annotation.text) {
                    var annotationContent = $("<div>").css({
                        "background": "#f9f9f9",
                        ...
                        "font-size": "0.9em",
                        "margin-bottom": "8px"
                    }).appendTo(annotationContainer);
                    
                    // Render the markdown annotation
                    var renderedAnnotation = RED.utils.renderMarkdown(annotation.text);
                    annotationContent.html(renderedAnnotation);
                    addTargetToExternalLinks(annotationContent);
                    
                    // Add edit button
                    var editButton = $('<button type="button" class="red-ui-button red-ui-button-small">').css({
                        "float": "right",
                        "margin-top": "4px"
                    }).text("Edit").on("click", function(evt) {
                        evt.preventDefault();
                        evt.stopPropagation();
                        if (RED.view.annotations && RED.view.annotations.setUserAnnotation) {
                            var currentText = RED.view.annotations.getUserAnnotation(node).text;
                            var newText = prompt('Edit annotation:', currentText);
                            if (newText !== null) {
                                RED.view.annotations.setUserAnnotation(node, newText);
