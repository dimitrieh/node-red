...
RED.sidebar.info = (function() {
    ...
    function resizeStack() {
        if (panels) {
            ...
            var parentHeight = contentParent.height();
            
            // Account for debug panel being open by checking sidebar bottom position
            var sidebar = $("#red-ui-sidebar");
            var sidebarBottom = parseInt(sidebar.css('bottom')) || 0;
            
            // If sidebar is adjusted for debug panel, we need to account for that
            if (sidebarBottom > 7) {
                // Debug panel is open, adjust available height
                var debugPanelHeight = sidebarBottom - 7;
                parentHeight = Math.max(100, parentHeight - debugPanelHeight);
            }
            
            var h = parentHeight - tipContainer.outerHeight();
            panels.resize(h)
        }
    }
    function init() {
        ...
        RED.events.on("bottomPanel:resize", resizeStack);
