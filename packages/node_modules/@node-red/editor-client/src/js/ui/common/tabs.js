...
RED.tabs = (function() {
    ...
    function createTabs(options) {
        ...
        if (options.collapsible) {
            ...
            if (options.menu !== false) {
                ...
                selectButton.on("click", function(evt) {
                    ...
                    // Always recreate menu to show only currently hidden tabs
                    if (collapsibleMenu) {
                    ...
                    // Only add tabs that are not visible in the button row
                    ul.children().each(function(i,el) {
                        var id = $(el).data('tabId');
                        var tabButton = $("#" + $(el).attr('id') + "-link-button");
                        var isActive = $(el).hasClass('active');

                        // Only add to dropdown if the button is hidden AND it's not the active tab
                        if (!isActive && (tabButton.length === 0 || !tabButton.is(":visible"))) {
                            var opt = {
                    ...
                    if (options.length > 0) {
                        ...
                        // No hidden tabs, don't show empty menu
                        return;
                    }
        ...
        function updateTabWidths() {
            ...
            if (options.collapsible) {
                // First, find the active tab's button
                var activeTab = ul.find("li.active");
                var activeTabId = activeTab.find("a").attr("href");
                if (activeTabId) {
                    activeTabId = activeTabId.slice(1); // Remove the # prefix
                }

                // Get all buttons and menu button
                var menuButton = collapsedButtonsRow.find(".red-ui-tab-link-button-menu");
                var hasMenuButton = menuButton.length > 0;
                var buttons = collapsedButtonsRow.find(".red-ui-tab-link-button:not(.red-ui-tab-link-button-menu)");
                var totalButtons = buttons.length;

                // Show all buttons initially to calculate actual space needs
                buttons.show();
                if (hasMenuButton) {
                    menuButton.hide(); // Hide menu initially
                }

                // Hide the active tab button
                if (activeTabId) {
                    $("#" + activeTab.attr("id") + "-link-button").hide();
                }

                // Calculate actual button row width with all visible buttons (except active)
                var currentButtonRowWidth = 0;
                buttons.each(function() {
                    if ($(this).is(":visible")) {
                        currentButtonRowWidth += $(this).outerWidth(true); // Include margins
                    }
                });

                // Calculate tab header width
                var tabHeaderWidth = 200; // Minimum space for tab header
                var maxTabWidth = 198;
                var minTabWidth = 120;

                // Available space for buttons
                var availableSpace = width - tabHeaderWidth - 20; // Leave padding

                // Check if all non-active buttons fit
                if (currentButtonRowWidth > availableSpace && hasMenuButton) {
                    // Need dropdown menu - show it
                    menuButton.show();
                    var menuButtonWidth = menuButton.outerWidth(true);

                    // Calculate how many buttons we can show with the menu
                    var spaceForButtons = availableSpace - menuButtonWidth;
                    var buttonsToHide = [];
                    var currentWidth = 0;

                    // Working from the end (right-most buttons), hide buttons that don't fit
                    buttons.get().reverse().forEach(function(btn) {
                        var $btn = $(btn);
                        if ($btn.is(":visible")) {
                            var btnWidth = $btn.outerWidth(true);
                            if (currentWidth + btnWidth > spaceForButtons) {
                                buttonsToHide.push($btn);
                            } else {
                    ...
                    // Hide the buttons that don't fit
                    buttonsToHide.forEach(function($btn) {
                        $btn.hide();
                    });

                } else {
                    // All buttons fit, no need for dropdown
                    if (hasMenuButton) {
                        menuButton.hide();
                    }
                }
