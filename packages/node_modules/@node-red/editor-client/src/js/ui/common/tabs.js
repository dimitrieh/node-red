    ...
    function createTabs(options) {
            ...
            if (options.menu !== false) {
                ...
                selectButton.on("click", function(evt) {
                    ...
                    if (!collapsibleMenu) {
                        ...
                        // Get the list of hidden button IDs from the menu button's data
                        var hiddenButtonIds = $(this).data("hiddenButtons") || [];

                        // Only add tabs that are hidden to the dropdown
                        ul.children().each(function(i,el) {
                            var id = $(el).data('tabId');
                            // Check if this tab's button is hidden
                            var isHidden = hiddenButtonIds.indexOf(id) !== -1;

                            // Only add to menu if it's hidden (not visible as a button)
                            if (isHidden && tabs[id]) {
                                var opt = {
                        ...
                        // If there are no hidden tabs, don't create an empty menu
                        if (options.length === 0) {
                            return;
                        }
        ...
        function activateTab(link) {
            ...
            if (!link.parent().hasClass("active")) {
                ...
                var activeButton = $("#"+parentId+"-link-button");
                activeButton.addClass("active selected");
                if (options.scrollable) {
        ...
        function updateTabWidths() {
            ...
            if (options.collapsible) {
                // Find the active tab to exclude it from visible buttons
                var activeTabId = ul.find("li.active").data("tabId");

                // Get all tab link buttons (not the dropdown menu button)
                var allButtons = collapsedButtonsRow.children(".red-ui-tab-link-button").not(".red-ui-tab-link-button-menu");
                var menuButton = collapsedButtonsRow.find(".red-ui-tab-link-button-menu");

                // First, show all buttons to calculate properly
                allButtons.show();

                // Calculate available space
                var availableWidth = width - 10; // Leave some padding
                var tabWidth = 200; // Default/desired tab width
                var buttonWidth = 32; // Approximate width per button icon

                // Calculate how much space we need for the tab
                var minTabWidth = 120;
                var maxTabWidth = 250;

                // Get all tabs that should have buttons (excluding active)
                var buttonsToDisplay = [];
                var activeButtonElement = null;

                allButtons.each(function() {
                    var buttonTabId = $(this).data("tabId");
                ...
                // Hide the active tab's button
                if (activeButtonElement) {
                    activeButtonElement.hide();
                }

                var totalButtonCount = buttonsToDisplay.length;
                var spaceForButtons = totalButtonCount * buttonWidth;
                var remainingSpace = availableWidth - spaceForButtons;

                // Determine if we need the dropdown
                if (remainingSpace >= minTabWidth) {
                    // We have enough space for all buttons and a reasonable tab
                    menuButton.hide();
                    buttonsToDisplay.forEach(function(btn) {
                        btn.element.show();
                    });
                    tabWidth = Math.min(maxTabWidth, remainingSpace);
                } else {
                    // We need to hide some buttons and show dropdown
                    menuButton.show();
                    var dropdownWidth = 32;
                    var maxButtons = Math.floor((availableWidth - minTabWidth - dropdownWidth) / buttonWidth);
                    maxButtons = Math.max(0, maxButtons); // Ensure non-negative

                    var hiddenButtonIds = [];

                    buttonsToDisplay.forEach(function(btn, index) {
                        if (index < maxButtons) {
                            btn.element.show();
                        } else {
                            btn.element.hide();
                            hiddenButtonIds.push(btn.id);
                        }
                    });

                    // Store hidden button IDs for menu creation
                    menuButton.data("hiddenButtons", hiddenButtonIds);

                    // Clear the menu so it gets recreated with updated content
                    if (collapsibleMenu) {
                ...
                // Clean up menu if not needed
                if (!menuButton.is(":visible") && collapsibleMenu) {
                    collapsibleMenu.remove();
                    collapsibleMenu = null;
                }

                // Calculate final tab width based on actual button row width
                var actualButtonsWidth = collapsedButtonsRow.width();
                tabWidth = Math.max(minTabWidth, width - actualButtonsWidth - 10);
