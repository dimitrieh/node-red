...
RED.sidebar = (function() {
    ...
    function init () {
        ...
        // Initialize floating window for sidebar
        setTimeout(function() {
            initFloatingWindow();
        }, 100);
        
        // Add action for floating sidebar toggle
        RED.actions.add("core:toggle-floating-sidebar", function() {
    ...
    function initFloatingWindow() {
        ...
        var sidebarElement = $("#red-ui-sidebar");
        var separatorElement = $("#red-ui-sidebar-separator");
        
        // Add floating class to enable floating styles
        sidebarElement.addClass('red-ui-sidebar-floating');
        separatorElement.addClass('red-ui-sidebar-floating');
        
        // Create floating window
        var floatingWindow = RED.floatingWindows.createWindow({
            id: 'red-ui-sidebar',
            title: 'Info', // Will be updated dynamically
        ...
        sidebarElement.data('floating-window', floatingWindow);
        
        // Store original onchange function and wrap it
        var originalOnChange = sidebar_tabs.onchange;
        sidebar_tabs.onchange = function(tab) {
            // Call original onchange logic
            if (originalOnChange) {
                originalOnChange.call(this, tab);
            }
            
            // Update floating window title
            var tabName = tab.label || tab.id;
            RED.floatingWindows.updateWindowTitle('red-ui-sidebar', tabName.charAt(0).toUpperCase() + tabName.slice(1));
        };
        
        // Update workspace positioning
        updateWorkspaceForFloatingSidebar(true);
    }
    
    function updateWorkspaceForFloatingSidebar(isFloating) {
        if (isFloating) {
            // Remove sidebar width from workspace calculation
            $('#red-ui-workspace').css('right', '0px');
            $('#red-ui-editor-stack').css('right', '0px');
        } else {
            // Restore original positioning
            $('#red-ui-workspace').css('right', '315px');
            $('#red-ui-editor-stack').css('right', '316px');
        }
    }
    
    function toggleFloatingSidebar() {
        var floatingWindow = $("#red-ui-sidebar").data('floating-window');
        if (floatingWindow && floatingWindow.is(':visible')) {
            RED.floatingWindows.closeWindow('red-ui-sidebar');
        } else {
            RED.floatingWindows.showWindow('red-ui-sidebar');
        }
    }
    ...
    return {
        ...
        toggleFloatingSidebar: toggleFloatingSidebar
    }
