...
RED.sidebar = (function() {
    ...
    var floatingSidebarWindow = null;

    function init () {
        // Create the sidebar content for floating window
        var sidebarContent = $('<div id="red-ui-sidebar" class="red-ui-floating-sidebar-content">');
        
        sidebar_tabs = RED.tabs.create({
            element: $('<ul id="red-ui-sidebar-tabs"></ul>').appendTo(sidebarContent),
            onchange:function(tab) {
                sidebarContent.find("#red-ui-sidebar-content").children().hide();
                sidebarContent.find("#red-ui-sidebar-footer").children().hide();
                if (tab.onchange) {
            ...
            order: RED.settings.get("editor.sidebar.order",["info", "help", "version-control"])  // Debug moved to separate window
        });

        $('<div id="red-ui-sidebar-content"></div>').appendTo(sidebarContent);
        $('<div id="red-ui-sidebar-footer" class="red-ui-component-footer"></div>').appendTo(sidebarContent);
        $('<div id="red-ui-sidebar-shade" class="hide"></div>').appendTo(sidebarContent);
        
        // Create the floating window
        floatingSidebarWindow = RED.floatingWindows.createFloatingWindow(
            sidebarContent,
            'Sidebar',
            {
                ...
                top: 20,
                width: 320,
                height: window.innerHeight - 120,  // Use almost all vertical space
        ...
        // Set the window ID for state management
        floatingSidebarWindow.attr('data-window-id', 'sidebar');

        // Create separate debug floating window
        var debugContent = $('<div id="red-ui-debug" class="red-ui-floating-debug-content">');
        debugContent.append('<div id="red-ui-debug-content">Debug messages will appear here</div>');
        
        var floatingDebugWindow = RED.floatingWindows.createFloatingWindow(
            debugContent,
            'Debug',
            {
                left: 250,  // Position in middle-left area
                top: 20,
                width: 360,  // Slightly wider than sidebar
                height: Math.floor((window.innerHeight - 120) * 0.6),  // Use 60% of available height
                className: 'red-ui-floating-debug',
                closable: false,
                minimizable: true
            }
        );
        floatingDebugWindow.attr('data-window-id', 'debug');
    ...
    function toggleFloatingWindow() {
        if (floatingSidebarWindow) {
            if (floatingSidebarWindow.is(':visible')) {
                floatingSidebarWindow.hide();
            } else {
                floatingSidebarWindow.show();
            }
        }
    }
    ...
    return {
        ...
        toggleSidebar: toggleFloatingWindow, // Use floating window toggle instead
