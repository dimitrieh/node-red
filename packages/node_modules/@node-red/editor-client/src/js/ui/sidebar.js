...
RED.sidebar = (function() {
    ...
    function init () {
        ...
        // Initialize floating window mode after DOM is ready
        setTimeout(() => {
            initializeFloatingSidebar();
        }, 200);
    }
    
    function initializeFloatingSidebar() {
        // Move sidebar content to floating window (excluding debug)
        var sidebarElement = $('#red-ui-sidebar');
        var debugTab = null;
        
        // Extract debug tab before moving sidebar
        if (knownTabs.debug) {
            debugTab = knownTabs.debug;
            // Remove debug from main sidebar
            sidebar_tabs.removeTab('debug');
        }
        
        sidebarElement.detach();
        
        // Create main floating sidebar window (without debug)
        RED.floatingWindows.create({
            id: 'red-ui-floating-sidebar-window',
            title: 'Sidebar',
            className: 'red-ui-floating-sidebar',
            content: sidebarElement,
            left: window.innerWidth - 340, // Position at right side
            top: 20,
            width: 320,
        ...
        if (debugTab) {
            var debugContent = $('<div class="red-ui-debug-window">');
            debugContent.append(debugTab.wrapper);
            
            RED.floatingWindows.create({
                id: 'red-ui-floating-debug-window',
                title: 'Debug',
            ...
            debugTab.wrapper.show();
            if (debugTab.toolbar) {
                $(debugTab.toolbar).show();
            }
        }
    }
