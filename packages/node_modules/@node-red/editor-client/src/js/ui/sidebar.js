...
RED.sidebar = (function() {
    ...
    var floatingWindowId = null;

    function initFloatingWindow() {
        // Create floating window for sidebar
        floatingWindowId = RED.floatingWindows.create({
            id: 'floating-sidebar-window',
            title: 'Sidebar',
            width: 350,
            height: 500,
            x: $(window).width() - 400,
            ...
            draggable: true,
            minimizable: true,
            closable: false, // Don't allow closing the sidebar
            content: '', // Will be populated after creation
        ...
        // Move the existing sidebar content into the floating window
        var sidebarContent = $("#red-ui-sidebar").detach();
        RED.floatingWindows.updateContent(floatingWindowId, '');
        RED.floatingWindows.getWindow(floatingWindowId).element
            .find('.red-ui-floating-window-content')
            .append(sidebarContent);

        // Add floating window specific class
        RED.floatingWindows.getWindow(floatingWindowId).element
            .addClass('red-ui-floating-sidebar-window');

        // Update main container to be full width
        $("#red-ui-main-container").addClass("red-ui-sidebar-floating");
        
        // Update window title based on active tab
        var activeTab = sidebar_tabs.activeTab();
        if (activeTab && activeTab.label) {
            RED.floatingWindows.setTitle(floatingWindowId, activeTab.label);
        }
    }

    function toggleFloatingSidebar(visible) {
        if (floatingWindowId) {
            if (visible === undefined) {
                // Toggle current state
                var window = RED.floatingWindows.getWindow(floatingWindowId);
                visible = !window.element.is(':visible');
            }
            
            if (visible) {
                RED.floatingWindows.show(floatingWindowId);
            } else {
                RED.floatingWindows.hide(floatingWindowId);
            }
        }
    }
    ...
    return {
        init: init,
        initFloating: initFloatingWindow,
        toggleFloating: toggleFloatingSidebar,
        addTab: addTab,
        removeTab: removeTab,
        show: showSidebar,
