...
RED.sidebar = (function() {
    ...
    function init () {
        ...
        var debugHeader = $('<div id="red-ui-sidebar-debug-header"></div>').appendTo(lowerSection);
        // Add debug icon and label to header
        $('<i class="fa fa-bug"></i>').appendTo(debugHeader);
        $('<span>').text(' Debug').appendTo(debugHeader);
        ...
        $("#red-ui-sidebar-splitter").draggable({
            axis: "y",
            containment: "#red-ui-sidebar",
            helper: function() {
                // Create invisible helper to prevent CSS top jumping
                return $('<div>').css({
                    position: 'absolute',
                    width: '100%',
                    height: '7px',
            ...
            start: function(event, ui) {
                ...
                var upperHeight = $("#red-ui-sidebar-upper").height();
                $(this).data('startUpperHeight', upperHeight);
            ...
            drag: function(event, ui) {
                ...
                var minPos = minUpperHeight;
                var maxPos = sidebarHeight - Math.max(minLowerHeight, splitterHeight);

                // Apply 60% constraint
                var maxPosForPercent = sidebarHeight - maxLowerHeight - splitterHeight;
                maxPos = Math.min(maxPos, maxPosForPercent);

                // Constrain position
                var newPos = Math.max(minPos, Math.min(maxPos, ui.position.top));

                // Update section heights
                $("#red-ui-sidebar-upper").css('flex', 'none').height(newPos);
                $("#red-ui-sidebar-lower").height(sidebarHeight - newPos - splitterHeight);

                // Trigger resize events
                RED.events.emit("sidebar:resize");

                // Return false to prevent the splitter element from moving
                return false;
            },
            stop: function(event, ui) {
                $(this).removeClass('dragging');
                // Ensure no top style is left
                $(this).css('top', '');
        ...
        function handleResponsiveSidebar() {
            ...
            if (isSmallViewport && !debugPanelInTabs) {
                // Store debug content references
                debugContent = $("#red-ui-sidebar-debug-content").children();
                debugFooter = $("#red-ui-sidebar-debug-footer").children();
                ...
                // Add as first tab
                sidebar_tabs.addTab(debugTab, 0);
                ...
                $("#red-ui-sidebar-debug-content").append(debugContent);
                $("#red-ui-sidebar-debug-footer").append(debugFooter);
        ...
        // Don't run initial check to avoid hiding debug panel on page load
        // handleResponsiveSidebar(); // Initial check
    }
    ...
    var debugPanel = (function() {
        ...
        function init(options) {
            // Create debug header with icon
            var header = $('#red-ui-sidebar-debug-header');
            header.empty();
            $('<i class="' + options.iconClass + '">').appendTo(header);
            $('<span>').text(options.label).appendTo(header);

            // Append content and footer
