 ...
 RED.view.navigator = (function() {
    ...
    var isAutoShowing = false;
     ...
     function autoShow() {
        ...
        if (!isShowing && !isAutoShowing) {
            isAutoShowing = true;
            resizeNavBorder();
            refreshNodes();
            navContainer.addClass('auto-fade').fadeIn(300);
        }

        autoHideTimeout = setTimeout(function() {
            if (isAutoShowing && !isShowing) {
                navContainer.fadeOut(300, function() {
                    navContainer.removeClass('auto-fade');
                    isAutoShowing = false;
                });
            }
        }, 2000);
    ...
    function toggle() {
         if (!isShowing) {
             ...
             navContainer.removeClass('auto-fade').stop(true, true).fadeIn(200);
         } else {
             isShowing = false;
             navContainer.stop(true, true).fadeOut(100);
     ...
     return {
         init: function() {
             ...
             // Listen for canvas interaction events to auto-show minimap
             RED.events.on("view:pan", autoShow);
             RED.events.on("view:zoom", autoShow);
             RED.events.on("view:scroll", autoShow);

             // Show briefly on startup
             setTimeout(function() {
                 autoShow();
             }, 500);
