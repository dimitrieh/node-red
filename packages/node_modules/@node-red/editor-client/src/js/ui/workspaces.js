...
RED.workspaces = (function() {
    ...
    // Breadcrumb navigation variables
    var breadcrumbTrail = [];

    function updateBreadcrumbs() {
        const breadcrumbContainer = $("#red-ui-breadcrumb-bar");
        const breadcrumbList = $("#red-ui-breadcrumb-bar .red-ui-breadcrumb-list");
        
        // Check if we're in a subflow
        const currentWorkspace = RED.nodes.subflow(activeWorkspace);
        if (!currentWorkspace) {
            // Hide breadcrumbs in main flows (not subflows)
            breadcrumbContainer.hide();
            breadcrumbTrail = [];
            return;
        }

        // Build breadcrumb trail for subflows
        breadcrumbTrail = [];
        
        // Add main flows entry
        breadcrumbTrail.push({
            id: 'main',
            name: RED._('workspace.defaultName', {number: 1}) || 'Main Flow',
            type: 'tab'
        });
        ...
        breadcrumbTrail.push({
            ...
            name: currentWorkspace.label || currentWorkspace.name || 'Subflow',
        ...
        for (let i = 0; i < breadcrumbTrail.length; i++) {
            const crumb = breadcrumbTrail[i];
            const isLast = (i === breadcrumbTrail.length - 1);
            
            const li = $('<li class="red-ui-breadcrumb-item"></li>').appendTo(breadcrumbList);
            
            if (!isLast) {
                const link = $('<a href="#" class="red-ui-breadcrumb-link">' + crumb.name + '</a>').appendTo(li);
                link.on('click', function(e) {
                    e.preventDefault();
                    if (crumb.id === 'main') {
                        // Navigate to first available flow tab
                        var workspaces = RED.nodes.getWorkspaces();
                        if (workspaces.length > 0) {
                            RED.workspaces.show(workspaces[0].id);
                        }
                    } else {
                        RED.workspaces.show(crumb.id);
                    }
                });
                
                // Add separator
                $('<li class="red-ui-breadcrumb-separator">›</li>').appendTo(breadcrumbList);
            } else {
                // Current page - no link
                $('<span class="red-ui-breadcrumb-current">' + crumb.name + '</span>').appendTo(li);
            }
        }
    ...
    function createWorkspaceTabs() {
        workspace_tabs = RED.tabs.create({
            ...
            onchange: function(tab) {
                ...
                updateBreadcrumbs(); // Update breadcrumb navigation
                RED.sidebar.config.refresh();
                RED.view.focus();
            },
    ...
    function init() {
        ...
        $('<div id="red-ui-breadcrumb-bar" class="red-ui-breadcrumb-container" style="display:none;"><nav class="red-ui-breadcrumb-nav"><ol class="red-ui-breadcrumb-list"></ol></nav></div>').appendTo("#red-ui-workspace");
