...
RED.editor = (function() {
    ...
    function showEditDialog(node, defaultTab) {
        ...
        // Check if there's another editor open with unsaved changes
        if (editStack.length > 0 && hasUnsavedChanges) {
            RED.notify.confirm({
                message: "You have unsaved changes in the current editor. Do you want to save them?",
                buttons: [
                    {
                        text: "Save",
                        class: "primary",
                        click: function() {
                            // Save current editor then open the new one
                            $("#node-dialog-ok").trigger("click");
                            setTimeout(function() {
                                RED.editor.edit(node, defaultTab);
                            }, 300);
                        }
                    },
                    {
                        text: "Discard",
                        click: function() {
                            // Close current editor without saving and open new one
                    ...
                    },
                    {
                        text: "Cancel",
                        click: function() {
                            // Do nothing, keep current editor open
                        }
                    }
                ]
            });
        ...
        var trayOptions = {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        hasUnsavedChanges = false; // Reset flag after save
                        RED.tray.close();
                    }
                }
            ],
            ...
            open: function(tray, done) {
                ...
                // Capture original node state for change detection
                originalNodeState = JSON.stringify(editing_node);
                hasUnsavedChanges = false;
                ...
                prepareEditDialog(trayBody, nodeEditPanes,node,node._def,"node-input", defaultTab, function(_activeEditPanes) {
                    ...
                    // Attach change listeners to detect unsaved changes
                    setTimeout(function() {
                        trayBody.find("input, select, textarea").on("change input", function() {
    ...
    return {
        ...
        isEditing: function() { return editStack.length > 0; },
