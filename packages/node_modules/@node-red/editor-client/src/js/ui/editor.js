...
RED.editor = (function() {
    ...
    // Track unsaved changes in the editor
    ...
    function buildEditForm(container,formId,type,ns,node) {
        ...
        attachChangeListeners(dialogForm);
    ...
    function attachChangeListeners(form) {
        ...
        form.find('input, textarea, select').on('change input', function() {
            // Don't track changes to id fields or other system fields
            if (!$(this).attr('id') || $(this).attr('id').indexOf('red-ui-trap-') === 0) {
                return;
            }
            hasUnsavedChanges = true;
        });

        // Also track changes to custom editors (e.g., code editors)
        form.on('editor:change', function() {
            hasUnsavedChanges = true;
        });
    }
    ...
    // Function to show unsaved changes confirmation dialog
    function confirmUnsavedChanges(callback) {
        if (hasUnsavedChanges && currentEditingNode) {
                    ...
                    buttons: [
                        {
                            ...
                            click: function() {
                                notification.close();
                                // Trigger save on the current editor
                                $("#node-dialog-ok").click();
                                // Wait a bit for save to complete then execute callback
                        ...
                        },
                        {
                            ...
                            click: function() {
                                ...
                                RED.tray.close();
                                // Wait for tray to close then execute callback
                                setTimeout(callback, 100);
                            }
                        },
                        {
    ...
    function showEditDialog(node, defaultTab) {
        ...
        // Check for unsaved changes before opening a new editor
        if (hasUnsavedChanges && currentEditingNode && currentEditingNode !== node) {
            confirmUnsavedChanges(function() {
                showEditDialog(node, defaultTab);
            });
            return;
        }
        ...
        var trayOptions = {
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (hasUnsavedChanges) {
                            confirmUnsavedChanges(function() {
                                // User chose to save or discard, tray will be closed by the dialog
                            });
                            return;
                        }
            ...
            open: function(tray, done) {
                // Reset unsaved changes flag when opening editor
                hasUnsavedChanges = false;
            ...
            close: function() {
                // Reset unsaved changes tracking when closing
                hasUnsavedChanges = false;
