RED.sidebar.info.outliner = (function() {
    ...
    function getNodeLabel(n) {
        ...
        // Add hover event handlers to sync with view
        div.on("mouseenter", function() {
            // Trigger hover on the node in the view
            RED.view.hoverNode(n.id);
        });
        div.on("mouseleave", function() {
            // Clear hover on the node in the view
            RED.view.hoverNode(null);
        });
    ...
    function build() {
        ...
        treeList.on('treelistselect', function(e,item) {
            ...
            if (node) {
                ...
                if (node.type === 'group' || (node._def && node._def.category !== "config")) {
                    RED.view.select({nodes:[node]})
                } else if (node._def && node._def.category === "config") {
                    RED.sidebar.info.refresh(node);
        ...
        // Auto-expand parent flow when a node is selected
        RED.events.on("view:selection-changed", function(selection) {
            if (selection.nodes && selection.nodes.length > 0) {
                var node = selection.nodes[0];
                // First check if we're in the outline tab
                if (RED.leftSidebar && RED.leftSidebar.getCurrentTab() === 'outliner') {
                    // Get the parent (flow or group) of the selected node
                    var parent = node.g || node.z;
                    if (parent && objects[parent]) {
                        // Expand the parent to show the selected node
                        treeList.treeList('show', objects[parent]);
                        // Also reveal the node itself
                        if (objects[node.id]) {
                    ...
                    // Still update selection even if not in outline tab
                    if (objects[node.id]) {
                        treeList.treeList('select', objects[node.id]);
                    }
                }
        ...
        // Handle hover sync from view to outline
        RED.events.on("view:node-hover", function(node) {
            if (node && objects[node.id]) {
                objects[node.id].element.addClass("red-ui-info-outline-item-hover");
            }
        });
        
        RED.events.on("view:node-hover-out", function(node) {
            if (node && objects[node.id]) {
                objects[node.id].element.removeClass("red-ui-info-outline-item-hover");
            }
        });
