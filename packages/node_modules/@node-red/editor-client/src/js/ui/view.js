...
RED.view = (function() {
    ...
    function init() {
            ...
            .on("touchmove", function(){
                    ...
                    if (d3.event.touches.length<2) {
                        ...
                        if (!isNaN(moveTouchDistance)) {
                            ...
                            // Calculate the focal point for pinch zoom (center of two fingers)
                            var focalPoint = [
                                touchCenter[0] - offset.left + scrollPos[0],
                                touchCenter[1] - offset.top + scrollPos[1]
                            ];

                            // Use cursor-centered zoom for pinch
        ...
        // Track spacebar for space+scroll zoom
        const handleSpacebarToggle = (event) => {
        ...
        d3.select(document).on('keydown.space', handleSpacebarToggle);
        d3.select(document).on('keyup.space', handleSpacebarToggle);
        ...
        // Enhanced mousewheel handler with space+scroll and cursor-centered zoom
        chart.on("DOMMouseScroll mousewheel wheel", function (evt) {
            // Handle both Alt+scroll and Space+scroll for zoom
            if (evt.altKey || spacebarPressed || evt.ctrlKey) {
                evt.preventDefault();
                evt.stopPropagation();
                ...
                if (evt.originalEvent.deltaY) {
                    ...
                    // Legacy mousewheel
                    delta = -(evt.originalEvent.detail) || evt.originalEvent.wheelDelta;
                }

                // Calculate new scale factor
                var newScale = scaleFactor;
                if (delta > 0) {
                    newScale = Math.min(2, scaleFactor + 0.1);
                } else {
                    newScale = Math.max(0.3, scaleFactor - 0.1);
                }
                ...
                if (lastScrollTime > 0) {
                    ...
                    if (timeDelta > 0 && timeDelta < 100) {
                        scrollVelocity.x = (currentScrollPos.x - lastScrollPos.x) / timeDelta * 16;
                        scrollVelocity.y = (currentScrollPos.y - lastScrollPos.y) / timeDelta * 16;
                ...
                // Start momentum scrolling when user stops
                clearTimeout(window.scrollEndTimer);
                window.scrollEndTimer = setTimeout(function() {
                    if (isUserScrolling && (Math.abs(scrollVelocity.x) > MIN_VELOCITY || Math.abs(scrollVelocity.y) > MIN_VELOCITY)) {
                        startMomentumScroll();
                    }
    ...
    function zoomView(factor, focalPoint) {
        ...
        // If focal point is provided, use it as the zoom center
        // Otherwise fall back to viewport center (legacy behavior)
        var center;
        if (focalPoint) {
            // Convert focal point from screen space to world space
            center = [focalPoint[0]/scaleFactor, focalPoint[1]/scaleFactor];
        } else {
            // Use viewport center
        ...
        // Calculate new position to keep the focal point stationary
        if (focalPoint) {
            // Keep the focal point at the same screen position
            var newWorldX = center[0] * scaleFactor;
            var newWorldY = center[1] * scaleFactor;
            var newScrollX = newWorldX - (focalPoint[0] - scrollPos[0]);
            var newScrollY = newWorldY - (focalPoint[1] - scrollPos[1]);
            chart.scrollLeft(newScrollX);
            chart.scrollTop(newScrollY);
        } else {
            // Legacy center-based zoom
    ...
    // Momentum scrolling with bounce effect
    function startMomentumScroll() {
        var element = chart[0];
        var maxScrollX = element.scrollWidth - element.clientWidth;
        var maxScrollY = element.scrollHeight - element.clientHeight;
        ...
        function animate() {
            ...
            var currentX = element.scrollLeft;
            var currentY = element.scrollTop;
            var targetX = currentX + scrollVelocity.x;
            var targetY = currentY + scrollVelocity.y;
            ...
            if (targetX < 0) {
                bounceX = -targetX;
                targetX = 0;
                scrollVelocity.x *= -BOUNCE_SPRING;
            } else if (targetX > maxScrollX) {
                bounceX = targetX - maxScrollX;
                targetX = maxScrollX;
                scrollVelocity.x *= -BOUNCE_SPRING;
            }
            ...
            if (targetY < 0) {
                bounceY = -targetY;
                targetY = 0;
                scrollVelocity.y *= -BOUNCE_SPRING;
            } else if (targetY > maxScrollY) {
                bounceY = targetY - maxScrollY;
                targetY = maxScrollY;
                scrollVelocity.y *= -BOUNCE_SPRING;
            }

            // Apply bounce offset with limits
            if (bounceX > 0) {
                targetX -= Math.min(bounceX * BOUNCE_SPRING, MAX_BOUNCE);
            }
            if (bounceY > 0) {
                targetY -= Math.min(bounceY * BOUNCE_SPRING, MAX_BOUNCE);
            }
            ...
            // Apply damping
            scrollVelocity.x *= BOUNCE_DAMPING;
            scrollVelocity.y *= BOUNCE_DAMPING;
