    ...
    function init() {
        ...
        outer = d3.select("#red-ui-workspace-chart")
            ...
            .on("mousedown", function() {
                focusView();
                if (spaceKeyPressed && d3.event.button === 0) {
            ...
            .on("mouseup", function() {
                if (spaceKeyPressed && mouse_mode === RED.state.PANNING) {
                    mouse_mode = 0;
                    outer.style("cursor", "grab");
                }
            })
            .on("mousemove", function() {
                if (spaceKeyPressed && mouse_mode === RED.state.PANNING && d3.event.buttons === 1) {
                    d3.event.preventDefault();
                    var deltaPos = [
                        mouse_position[0] - d3.event.pageX,
                        mouse_position[1] - d3.event.pageY
                    ];
            ...
            .on("touchmove", function(){
                    ...
                    if (d3.event.touches.length<2) {
                        ...
                        if (!isNaN(moveTouchDistance)) {
                            ...
                            // Calculate zoom with pinch center as focus
                            var factor = scaleFactor / oldScaleFactor;
                            var deltaTouchCenter = [
                                (startTouchCenter[0] - scrollPos[0]) * (factor - 1),
                                (startTouchCenter[1] - scrollPos[1]) * (factor - 1)
                            ];
        ...
        // Space key handler for grab-to-pan
        var handleSpaceKey = function(evt) {
            if (evt.target.tagName === "INPUT" || evt.target.tagName === "TEXTAREA") {
                return;
            }
            if (evt.keyCode === 32) { // Space key
                if (evt.type === "keydown" && !spaceKeyPressed) {
                    spaceKeyPressed = true;
                    // Change cursor to grab
                    outer.style("cursor", "grab");
                    evt.preventDefault();
                } else if (evt.type === "keyup" && spaceKeyPressed) {
        ...
        chart.on("DOMMouseScroll mousewheel", function (evt) {
            ...
            var delta = evt.originalEvent.deltaY || -evt.originalEvent.wheelDelta || evt.originalEvent.detail;

            if (evt.altKey || evt.ctrlKey || evt.metaKey) {
                // Zoom mode
                var move = -delta;

                // Get mouse position relative to canvas for cursor-centric zoom
                var mousePos = d3.mouse(outer.node());
                var previousScale = scaleFactor;
                ...
                if (previousScale !== scaleFactor) {
                    // Cursor-centric zoom
                    var factor = scaleFactor / previousScale;
                    var currentScrollLeft = chart.scrollLeft();
                    var currentScrollTop = chart.scrollTop();

                    var newScrollLeft = mousePos[0] * (factor - 1) + currentScrollLeft * factor;
                    var newScrollTop = mousePos[1] * (factor - 1) + currentScrollTop * factor;
                ...
                // Regular scroll with momentum
                var scrollX = evt.originalEvent.deltaX || 0;
                var scrollY = delta;
                ...
                if (timeDelta > 0) {
                    scrollVelocity.x = scrollX / timeDelta * 16;
                    scrollVelocity.y = scrollY / timeDelta * 16;
                }
                ...
                chart.scrollLeft(currentScrollLeft + scrollX);
                chart.scrollTop(currentScrollTop + scrollY);
                ...
                var animateMomentum = function() {
                    if (Math.abs(scrollVelocity.x) > 0.5 || Math.abs(scrollVelocity.y) > 0.5) {
                        var friction = 0.95;
                        scrollVelocity.x *= friction;
                        scrollVelocity.y *= friction;

                        var newLeft = chart.scrollLeft() + scrollVelocity.x;
                        var newTop = chart.scrollTop() + scrollVelocity.y;

                        // Edge bounce effect
                        var maxScrollLeft = outer.node().scrollWidth - chart.width();
                        var maxScrollTop = outer.node().scrollHeight - chart.height();

                        if (newLeft < 0) {
                            newLeft = -newLeft * 0.3;
                            scrollVelocity.x *= -0.5;
                        } else if (newLeft > maxScrollLeft) {
                            newLeft = maxScrollLeft + (newLeft - maxScrollLeft) * 0.3;
                            scrollVelocity.x *= -0.5;
                        }

                        if (newTop < 0) {
                            newTop = -newTop * 0.3;
                            scrollVelocity.y *= -0.5;
                        } else if (newTop > maxScrollTop) {
                            newTop = maxScrollTop + (newTop - maxScrollTop) * 0.3;
                            scrollVelocity.y *= -0.5;
                        }

                        chart.scrollLeft(newLeft);
                        chart.scrollTop(newTop);
