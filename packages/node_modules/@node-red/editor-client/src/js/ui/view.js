...
RED.view = (function() {
            ...
            if (movingSet.length() > 0) {
                ...
                if (historyEvent.events.length > 0) {
                    ...
                    if (RED.automerge && RED.automerge.isEnabled()) {
                        var movedNodes = [];
                        for (var mi = 0; mi < movingSet.length(); mi++) {
                            var mn = movingSet.get(mi);
                            if (mn.ox !== mn.n.x || mn.oy !== mn.n.y) {
                                movedNodes.push({id: mn.n.id, x: mn.n.x, y: mn.n.y});
                            }
                        }
                        if (movedNodes.length > 0) {
                            RED.automerge.batchPositionUpdate(movedNodes);
                        }
                    }
                    updateActiveNodes();
                }
    ...
    function zoomView(factor) {
        ...
        redraw();
        RED.events.emit('view:zoom', { scale: scaleFactor });
        if (RED.settings.get("editor.view.view-store-zoom")) {
