...
RED.view = (function() {
    ...
    // Generate secondary information for enhanced node anatomy
    function getNodeSecondaryInfo(node) {
        ...
        try {
            switch (node.type) {
                case 'inject':
                    if (node.topic) {
                        info = "topic: " + node.topic;
                    } else if (node.payload !== undefined && node.payloadType !== 'date') {
                        var payload = String(node.payload).substring(0, 20);
                        if (String(node.payload).length > 20) payload += "...";
                        info = payload;
                    } else if (node.repeat && node.repeat !== "" && node.repeat !== "0") {
                        info = "every " + node.repeat + "s";
                    } else {
                        info = "manual trigger";
                    }
                    break;
                case 'debug':
                    if (node.name) {
                        info = "to: " + (node.complete === 'payload' ? 'msg.payload' : node.complete);
                    } else {
                        info = node.tosidebar ? "sidebar" : "console";
                    }
                    break;
                case 'function':
                    if (node.func && node.func.length > 0) {
                        var firstLine = node.func.split('\n')[0].trim();
                ...
                case 'change':
                    if (node.rules && node.rules.length > 0) {
                        var rule = node.rules[0];
                        if (rule.t === 'set') {
                            info = rule.p + " = " + (rule.to || '').substring(0, 15);
                        } else if (rule.t === 'change') {
                            info = "replace in " + rule.p;
                    ...
                    break;
                case 'http in':
                    info = (node.method || 'get').toUpperCase() + " " + (node.url || '/');
                    break;
                case 'http request':
                    info = (node.method || 'GET') + " " + (node.url || 'url from msg');
                    break;
                case 'mqtt in':
                case 'mqtt out':
                    info = node.topic || 'topic from msg';
                    break;
                default:
                    // For other nodes, try to show some configuration info
                    if (node.topic) {
                        info = "topic: " + node.topic;
                    } else if (node.name && node.name !== node.type) {
                        info = node.name.substring(0, 25);
                    }
                    break;
            }
        } catch (e) {
            // Ignore errors in secondary info generation
            info = "";
        }
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
                ...
                subflowOutputs.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        if (mouse_mode != RED.state.MOVING_ACTIVE) {
                            ...
                            var yp = d.h / 2 - (this.__labelLineCount__ / 2) * 20 + 10;
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    // Generate secondary information for enhanced node anatomy
                    var secondaryInfo = getNodeSecondaryInfo(d);
                    var labelParts;
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__secondaryInfo__ !== secondaryInfo || this.__outputs__.length !== d.outputs) {
                        labelParts = getLabelParts(label, "red-ui-flow-node-label");
                        var totalLines = labelParts.lines.length + (secondaryInfo ? 1 : 0);
                        if (totalLines !== this.__labelLineCount__ || this.__label__ !== label || this.__secondaryInfo__ !== secondaryInfo) {
                            d.resize = true;
                        }
                        this.__label__ = label;
                        this.__secondaryInfo__ = secondaryInfo;
                        this.__labelLineCount__ = totalLines;

                        if (hideLabel) {
                            d.h = Math.max(node_height,(d.outputs || 0) * 15);
                        } else {
                            // Account for primary label lines plus secondary info line
                            var totalLines = labelParts.lines.length + (secondaryInfo ? 1 : 0);
                            d.h = Math.max(6+20*totalLines,(d.outputs || 0) * 15, 50);
                        }
                        this.__hideLabel__ = hideLabel;
                    }
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (labelParts) {
                            // The label has changed - include primary label and secondary info
                            var sa = labelParts.lines;
                            var sn = labelParts.lines.length + (secondaryInfo ? 1 : 0);
                            var textLines = this.__textGroup__.childNodes;
                            while(textLines.length > sn) {
                                textLines[textLines.length-1].remove();
                            }
                            // Create primary label lines
                            for (var i=0; i<labelParts.lines.length; i++) {
                                if (i===textLines.length) {
                                    var line = document.createElementNS("http://www.w3.org/2000/svg","text");
                                    line.setAttribute("class","red-ui-flow-node-label-text");
                                    line.setAttribute("x",0);
                                    line.setAttribute("y",i*20);
                                    this.__textGroup__.appendChild(line);
                                }
                                textLines[i].textContent = sa[i];
                            }
                            // Create secondary information line
                            if (secondaryInfo) {
                                var secondaryIndex = labelParts.lines.length;
                                if (secondaryIndex === textLines.length) {
                                    var secondaryLine = document.createElementNS("http://www.w3.org/2000/svg","text");
                                    secondaryLine.setAttribute("class","red-ui-flow-node-secondary-text");
                                    secondaryLine.setAttribute("x",0);
                                    secondaryLine.setAttribute("y",secondaryIndex*20);
                                    this.__textGroup__.appendChild(secondaryLine);
                                }
                                textLines[secondaryIndex].textContent = secondaryInfo;
                            }
                        }
