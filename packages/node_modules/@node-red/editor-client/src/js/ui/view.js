...
RED.view = (function() {
    ...
    // Variables for enhanced navigation
    ...
    function init() {
        ...
        // Handle space key for grab-to-pan mode
        function handleSpaceKey(evt) {
            if (evt.keyCode === 32 && !$(evt.target).is(':input')) { // Space key
                evt.preventDefault();
                if (evt.type === "keydown" && !spacePressed) {
                    spacePressed = true;
                    chart.addClass("grab-mode");
                } else if (evt.type === "keyup" && spacePressed) {
                    spacePressed = false;
                    chart.removeClass("grab-mode");
                    if (isDraggingCanvas) {
        ...
        // Emit scroll event when canvas scrolls
        chart.on("scroll", function() {
            RED.events.emit("view:scroll");
        });

        chart.on("DOMMouseScroll mousewheel", function (evt) {
            if ( evt.altKey || spacePressed ) {
                evt.preventDefault();
                evt.stopPropagation();

                // Get cursor position relative to canvas
                var offset = chart.offset();
                var cursorX = evt.originalEvent.pageX - offset.left;
                var cursorY = evt.originalEvent.pageY - offset.top;

                var move = -(evt.originalEvent.detail) || evt.originalEvent.wheelDelta;

                // Zoom with cursor as focal point
                ...
                if (scaleFactor !== oldScale) {
                    ...
                    var scaleRatio = scaleFactor / oldScale;
                    var newScrollLeft = cursorX * scaleRatio - cursorX + chart.scrollLeft();
                    var newScrollTop = cursorY * scaleRatio - cursorY + chart.scrollTop();

                    chart.scrollLeft(newScrollLeft);
                    chart.scrollTop(newScrollTop);
                }
            } else {
                // Normal scrolling - emit scroll event for minimap
                RED.events.emit("view:scroll");
            }
        });
    ...
    function canvasMouseDown() {
        ...
        // Handle grab-to-pan with space key
        if (spacePressed && d3.event.button === 0) {
            d3.event.preventDefault();
            d3.event.stopPropagation();
            isDraggingCanvas = true;
            chart.removeClass("grab-mode").addClass("grabbing");
            lastMouseX = d3.event.pageX;
            lastMouseY = d3.event.pageY;
            velocityX = 0;
            velocityY = 0;
            if (momentumActive) {
    ...
    function canvasMouseMove() {
        ...
        // Handle grab-to-pan dragging
        if (isDraggingCanvas && spacePressed) {
            d3.event.preventDefault();
            var deltaX = d3.event.pageX - lastMouseX;
            var deltaY = d3.event.pageY - lastMouseY;
            ...
            // Emit pan event for minimap
            RED.events.emit("view:pan");
            return;
        }
    ...
    function canvasMouseUp() {
        ...
        // Handle grab-to-pan release
        if (isDraggingCanvas) {
            isDraggingCanvas = false;
            if (!spacePressed) {
                chart.removeClass("grabbing");
            } else {
                chart.removeClass("grabbing").addClass("grab-mode");
            }
            startMomentum();
            return;
        }
    ...
    function startMomentum() {
        ...
        momentumActive = true;
        var friction = 0.95; // Deceleration factor

        function animateMomentum() {
            if (Math.abs(velocityX) > 0.1 || Math.abs(velocityY) > 0.1) {
                chart.scrollLeft(chart.scrollLeft() - velocityX);
                chart.scrollTop(chart.scrollTop() - velocityY);
    ...
    function applyEdgeBounce() {
        var scrollLeft = chart.scrollLeft();
        var scrollTop = chart.scrollTop();
        var maxScrollLeft = chart[0].scrollWidth - chart.width();
        var maxScrollTop = chart[0].scrollHeight - chart.height();

        var bounceStrength = 0.3;
