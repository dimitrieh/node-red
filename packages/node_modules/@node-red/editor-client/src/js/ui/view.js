...
RED.view = (function() {
    ...
    function init() {
        ...
        const handleSpacebarToggle = (event) => {
            ...
            if (event.target && event.target.nodeName && event.target.nodeName.toLowerCase() === "textarea") {
                return; // Don't trigger if typing in a textarea
            }
            if (event.code === 'Space') {
                event.preventDefault();
                if (event.type === 'keydown' && !spacebarPressed) {
                    spacebarPressed = true;
                    $("#red-ui-workspace-chart").css("cursor", "grab");
                } else if (event.type === 'keyup' && spacebarPressed) {
                    spacebarPressed = false;
                    $("#red-ui-workspace-chart").css("cursor", "");
    ...
    function canvasMouseDown() {
        ...
        if (spacebarPressed && d3.event.button === 0) {
            ...
            mouse_position = [d3.event.pageX,d3.event.pageY]
            scroll_position = [chart.scrollLeft(),chart.scrollTop()];
            $("#red-ui-workspace-chart").css("cursor", "grabbing");
            return;
        }
    ...
    function canvasMouseUp() {
        ...
        if (mouse_mode === RED.state.PANNING) {
            resetMouseVars();
            // Reset cursor if we were panning with spacebar
            if (spacebarPressed) {
                $("#red-ui-workspace-chart").css("cursor", "grab");
            } else {
                $("#red-ui-workspace-chart").css("cursor", "");
