...
RED.view = (function() {
    ...
    function nodeMouseOver(d) {
        ...
        if (mouse_mode === 0 || mouse_mode === RED.state.SELECTING_NODE) {
            ...
            // Emit hover event for outline sync
            RED.events.emit("view:node-hover", d);
            clearTimeout(portLabelHoverTimeout);
    ...
    function nodeMouseOut(d) {
        ...
        // Emit hover out event for outline sync
        RED.events.emit("view:node-hover-out", d);
        clearTimeout(portLabelHoverTimeout);
        if (portLabelHover) {
            portLabelHover.remove();
            portLabelHover = null;
    ...
    function hoverNode(nodeId) {
        // Handle hover from outline view
        // First clear any existing outline hovers
        nodeLayer.selectAll(".red-ui-flow-node-outline-hovered").each(function() {
            this.classList.remove("red-ui-flow-node-outline-hovered");
        });
        
        if (nodeId) {
            var node = RED.nodes.node(nodeId);
            if (node) {
                var nodeElement = nodeLayer.selectAll(".red-ui-flow-node").filter(function(d) {
                    return d.id === nodeId;
                });
                if (nodeElement && nodeElement[0] && nodeElement[0][0]) {
                    // Add outline hover class
                    nodeElement[0][0].classList.add("red-ui-flow-node-outline-hovered");
                }
            }
        }
    }
