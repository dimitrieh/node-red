...
RED.view = (function() {
    ...
    function init() {
        ...
        // Spacebar tracking for space+scroll zoom
        const handleSpacebarToggle = (event) => {
            if (event.code === "Space" && !event.repeat) {
                spacebarPressed = event.type === "keydown";
                // Prevent page scroll when space is pressed
                if (spacebarPressed && document.activeElement === document.body) {
        ...
        chart.on("DOMMouseScroll mousewheel wheel", function (evt) {
            // Handle space+scroll or alt+scroll for zoom
            if (evt.altKey || spacebarPressed) {
                evt.preventDefault();
                evt.stopPropagation();
                var move = -(evt.originalEvent.detail) || evt.originalEvent.wheelDelta || -evt.originalEvent.deltaY;
                if (move <= 0) { zoomOut(); }
                else { zoomIn(); }
            }
            // Handle trackpad pinch (wheel + ctrlKey)
            else if (evt.ctrlKey && evt.type === "wheel") {
                evt.preventDefault();
                evt.stopPropagation();
