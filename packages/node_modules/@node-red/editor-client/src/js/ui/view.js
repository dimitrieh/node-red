...
RED.view = (function() {
    ...
    function init() {
            ...
            .on("touchmove", function(){
                    ...
                    if (d3.event.touches.length<2) {
                        ...
                        if (!isNaN(moveTouchDistance)) {
                            ...
                            var newScaleFactor = Math.min(2,Math.max(0.3, scaleFactor * (moveTouchDistance / startTouchDistance)));

                            // Convert touch center to workspace coordinates
                            var workspacePoint = [
                                (touchCenter[0] - offset.left + scrollPos[0]) / scaleFactor,
                                (touchCenter[1] - offset.top + scrollPos[1]) / scaleFactor
                            ];

                            scaleFactor = newScaleFactor;

                            // Calculate new scroll position to keep touch point centered
                            var newScrollLeft = workspacePoint[0] * scaleFactor - (touchCenter[0] - offset.left);
                            var newScrollTop = workspacePoint[1] * scaleFactor - (touchCenter[1] - offset.top);
        ...
        d3.select(document).on("keydown.view", function() {
            if (d3.event.keyCode === 32 && !spaceKeyPressed) { // spacebar
        ...
        // Enhanced wheel event handler with space+scroll and trackpad pinch support
        chart.on("DOMMouseScroll mousewheel wheel", function (evt) {
            if (evt.altKey || spaceKeyPressed || evt.ctrlKey) {
                evt.preventDefault();
                evt.stopPropagation();
                ...
                if (originalEvent.type === "wheel") {
                    // Modern wheel event (better trackpad support)
                    delta = -originalEvent.deltaY;
                    // Handle pinch gesture on trackpad (ctrlKey + wheel)
                    if (evt.ctrlKey) {
                        delta = delta * 3; // Amplify for smoother pinch zoom
                    }
                } else {
                    // Legacy mousewheel events
                    delta = -(originalEvent.detail) || originalEvent.wheelDelta;
                }
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode === RED.state.PANNING) {
            ...
            var newScrollLeft = scroll_position[0]+deltaPos[0];
            var newScrollTop = scroll_position[1]+deltaPos[1];

            // Track velocity for bounce effect
            var currentTime = Date.now();
            var deltaTime = currentTime - lastScrollTime;
            if (deltaTime > 0) {
                scrollVelocity.x = (newScrollLeft - lastScrollPos.x) / deltaTime * 10;
                scrollVelocity.y = (newScrollTop - lastScrollPos.y) / deltaTime * 10;
            }
            lastScrollTime = currentTime;
            lastScrollPos = {x: newScrollLeft, y: newScrollTop};
    ...
    function canvasMouseUp() {
        ...
        if (mouse_mode === RED.state.PANNING) {
            // Apply bounce animation if there's velocity
            if (Math.abs(scrollVelocity.x) > 0.5 || Math.abs(scrollVelocity.y) > 0.5) {
    ...
    function zoomIn(cursorPos) {
        if (scaleFactor < 2) {
            zoomView(scaleFactor+0.1, cursorPos);
        }
    }
    function zoomOut(cursorPos) {
        if (scaleFactor > 0.3) {
            zoomView(scaleFactor-0.1, cursorPos);
        }
    }
    ...
    function applyBounceAnimation() {
        if (bounceAnimation) cancelAnimationFrame(bounceAnimation);
        ...
        var canvasWidth = space_width * scaleFactor;
        var canvasHeight = space_height * scaleFactor;
        var viewportWidth = chart.width();
        var viewportHeight = chart.height();
        var maxScrollX = Math.max(0, canvasWidth - viewportWidth);
        var maxScrollY = Math.max(0, canvasHeight - viewportHeight);
        ...
        function animate() {
            ...
            // Apply velocity
            scrollX += scrollVelocity.x;
            scrollY += scrollVelocity.y;

            // Check boundaries and apply bounce
            if (scrollX < 0) {
                scrollX = 0;
                scrollVelocity.x *= -bounce;
            ...
            chart.scrollLeft(scrollX);
            chart.scrollTop(scrollY);

            // Continue animation if velocity is significant
            if (Math.abs(scrollVelocity.x) > minVelocity || Math.abs(scrollVelocity.y) > minVelocity) {
    ...
    function zoomView(factor, cursorPos) {
        ...
        if (cursorPos) {
            ...
            center = [
                (scrollPos[0] + cursorPos[0]) / scaleFactor,
                (scrollPos[1] + cursorPos[1]) / scaleFactor
            ];
        } else {
            // Use viewport center (existing behavior)
        ...
        var newScrollLeft, newScrollTop;
        if (cursorPos) {
            newScrollLeft = center[0] * scaleFactor - cursorPos[0];
            newScrollTop = center[1] * scaleFactor - cursorPos[1];
        } else {
            ...
            newScrollLeft = scrollPos[0]-delta[0];
            newScrollTop = scrollPos[1]-delta[1];
        }
