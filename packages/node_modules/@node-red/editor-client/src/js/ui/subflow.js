...
RED.subflow = (function() {
    ...
    function deleteSubflow(id) {
        ...
        function completeDelete() {
            ...
            if (RED.automerge && RED.automerge.isEnabled()) {
                RED.automerge.removeNode(subflow.id, 'subflow');
            }
        }
    }
    ...
    function convertToSubflow() {
        ...
        if (RED.automerge && RED.automerge.isEnabled()) {
            RED.automerge.addNode(subflow);
            // Update converted nodes — they've been moved into the subflow tab
            for (var ci = 0; ci < nodeList.length; ci++) {
                RED.automerge.updateNode(nodeList[ci].id, {z: subflow.id, x: nodeList[ci].x, y: nodeList[ci].y});
            }
            // Add the subflow instance node
            RED.automerge.addNode(RED.nodes.nodeToExportable(subflowInstance));
        }
