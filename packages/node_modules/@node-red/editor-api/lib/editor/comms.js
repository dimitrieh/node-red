...
function CommsConnection(ws, user) {
    ...
    ws.on('message', function(data,flags) {
        ...
        if (Buffer.isBuffer(data) && data.length >= 5 && data[0] === 1) {
            handleBinaryMessage(self, data);
            return;
        }
 ...
 */
CommsConnection.prototype.sendBinary = function(topic, data) {
    try {
        const topicBuffer = Buffer.from(topic, 'utf8');
        const message = Buffer.alloc(1 + 4 + topicBuffer.length + data.length);

        // Type byte (1 = binary message)
        message[0] = 1;

        // Topic length (4 bytes, big-endian)
        message.writeUInt32BE(topicBuffer.length, 1);

        // Topic
        topicBuffer.copy(message, 5);

        // Payload
        if (Buffer.isBuffer(data)) {
            data.copy(message, 5 + topicBuffer.length);
        } else {
            Buffer.from(data).copy(message, 5 + topicBuffer.length);
        }
 ...
 * @param {CommsConnection} connection - The connection that received the message
 * @param {Buffer} data - Binary message data
 */
function handleBinaryMessage(connection, data) {
    ...
    const topicLen = data.readUInt32BE(1);
    if (data.length < 5 + topicLen) return;

    // Extract topic
    const topic = data.slice(5, 5 + topicLen).toString('utf8');

    // Extract payload
    const payload = data.slice(5 + topicLen);

    // Forward to runtime API
    runtimeAPI.comms.receive({
