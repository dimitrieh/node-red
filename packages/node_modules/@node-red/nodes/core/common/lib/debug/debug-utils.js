...
RED.debug = (function() {
    ...
    function init(_config) {
        ...
        var toolbar = $('<div class="red-ui-sidebar-header red-ui-debug-toolbar">'+
            '<div class="red-ui-debug-toolbar-left">'+
                '<i class="fa fa-bug"></i>'+
                '<span class="red-ui-debug-title-full">Debug output</span>'+
                '<span class="red-ui-debug-title-short">Debug</span>'+
            '</div>'+
            '<div class="red-ui-debug-toolbar-right">'+
                ...
                '</span>'+
                '<span class="button-group red-ui-debug-split-button">'+
                    '<a id="red-ui-sidebar-debug-clear" style="border-right: 1px solid var(--red-ui-primary-border-color); padding-right: 6px" class="red-ui-sidebar-header-button" href="#" data-clear-type="all" title="Clear messages"><i class="fa fa-trash"></i></a>' +
                    '<a id="red-ui-sidebar-debug-clear-opts" style="padding: 5px 8px; border-left: none;" class="red-ui-sidebar-header-button" href="#"><i class="fa fa-caret-down"></i></a>'+
                '</span>'+
            '</div>'+
        ...
        // Add dismissable message for undeployed nodes
        var undeployedMessage = $('<div class="red-ui-debug-undeployed-message hide">'+
            '<div class="red-ui-debug-undeployed-content">'+
                '<i class="fa fa-warning"></i> '+
                '<span>There are undeployed debug nodes that would show messages here</span>'+
                '<button class="red-ui-debug-undeployed-close"><i class="fa fa-times"></i></button>'+
            '</div>'+
        '</div>').appendTo(content);

        undeployedMessage.find('.red-ui-debug-undeployed-close').on('click', function() {
            undeployedMessage.slideUp(200);
            localStorage.setItem('debug-undeployed-dismissed', 'true');
        });
        ...
        // Primary clear button - always clears all messages
        ...
        // Dropdown button - shows menu with clear options
        toolbar.find("#red-ui-sidebar-debug-clear-opts").on("click", function(e) {
            e.preventDefault();
            e.stopPropagation();

            var hasFilter = (filterType === 'filterSelected' && Object.keys(filteredNodes).length > 0);

            var options = [
                { label: RED._('node-red:debug.sidebar.clearLog'), value: "all" },
                { label: RED._('node-red:debug.sidebar.clearFilteredLog'), value: "filtered", disabled: !hasFilter }
            ];
            ...
            var menu = RED.popover.menu({
                ...
                onselect: function(item) {
                    if (item.value === "all") {
                        clearMessageList();
                    } else if (item.value === "filtered" && !item.disabled) {
                        clearMessageList(true, true); // Clear only filtered messages
                    }
                }
            });
            menu.show({
        ...
        // Function to check for undeployed debug nodes
        function checkUndeployedNodes() {
            var hasUndeployedDebugNodes = false;
            // Check for undeployed debug nodes
            if (RED.nodes) {
                RED.nodes.eachNode(function(node) {
                    if (node.type === 'debug' && node.changed) {
                        hasUndeployedDebugNodes = true;
                    }
                });
            }

            var isDismissed = localStorage.getItem('debug-undeployed-dismissed') === 'true';
            if (hasUndeployedDebugNodes && !isDismissed) {
                undeployedMessage.slideDown(200);
            } else {
                undeployedMessage.slideUp(200);
            }
        }
        ...
        if (RED.events) {
            RED.events.on("deploy", function() {
                localStorage.removeItem('debug-undeployed-dismissed');
                checkUndeployedNodes();
            });

            RED.events.on("nodes:change", checkUndeployedNodes);
        }

        // Initial check
        setTimeout(checkUndeployedNodes, 500);
    }
