...
var api = module.exports = {
    ...
    */
     ...
     * @param {Boolean} opts.binary - whether this is a binary message
     * @return {Promise<Object>} - resolves when complete
     */
    ...
    /**
     * Send a binary message to a specific client or broadcast
     * @param {Object} opts
     * @param {String} opts.topic - the message topic
     * @param {Buffer|Uint8Array} opts.data - the binary data
     * @param {String} [opts.session] - specific session to send to
     * @param {String} [opts.excludeSession] - session to exclude from broadcast
     */
    sendBinary: async function(opts) {
        connections.forEach(connection => {
            if ((!opts.session || connection.session === opts.session) &&
                (!opts.excludeSession || connection.session !== opts.excludeSession)) {
                if (connection.sendBinary) {
                    connection.sendBinary(opts.topic, opts.data);
                }
            }
        });
    }
