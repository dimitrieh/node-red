...
var localfilesystem = {
    init: async function(_settings, _runtime) {
        ...
        // Initialize Automerge if collaborative editing is enabled
        if (settings.collaborativeEditing) {
            automerge.init(runtime);
        }
    ...
    getFlows: async function() {
        // Get flows from traditional storage
        const flows = await projects.getFlows();

        // If collaborative editing is enabled, load Automerge document
        if (settings.collaborativeEditing && automerge.isEnabled()) {
            await automerge.loadDocument(settings.userDir, flows);
        }
    ...
    saveFlows: async function(flows, user) {
        // Always save to traditional JSON storage (dual-write mode)
        await projects.saveFlows(flows, user);

        // Also update Automerge document if enabled
        if (settings.collaborativeEditing && automerge.isEnabled()) {
            automerge.setFlows(flows);
        }
    },
    ...
    // Expose Automerge module for direct access
    automerge: automerge
};
